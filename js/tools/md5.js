...
const MD5Tool = {
    ...
    generate() {
        const input = document.getElementById(SELECTORS.md5Input).value;
        if (!input) {
            Utils.showToast('请输入要加密的文本', 'error');
            return;
        }

        const md5Value = this.md5(input);
        document.getElementById(SELECTORS.md5Value).textContent = md5Value;
        Utils.toggleElement(SELECTORS.md5Result, true);
        Utils.showToast('MD5生成成功');
    },

    copy() {
        const value = document.getElementById(SELECTORS.md5Value).textContent;
        if (!value) return;
